...
import com.example.checkers.Othermode.GameEngine;
import com.example.checkers.Othermode.Utility;
    ...
    private static RoomDatabase.Callback sRoomDatabaseCallback =
            new RoomDatabase.Callback() {
                ...
                public void onCreate(@NonNull SupportSQLiteDatabase db) {
                    super.onCreate(db);
                    try {
                        ArrayList<com.example.checkers.Othermode.Item>[] ways = GameEngine.GetDefaultPosition();
                        for(int i = 0; i < ways.length; ++i) {
                            for(int j = 0; j < ways[i].size(); ++j) {
                                // Убираем все шашки с доски
                                ways[i].get(j).type = com.example.checkers.Othermode.Item.ITEM_TYPE.square;
                            }
                        }

                        JSONObject json = Utility.ParseJSONData("INIT_DB_STATE.json");
                        String gameName  = json.getString("game_name");
                        int turn = com.example.checkers.Othermode.Item.ITEM_TYPE.valueOf(json.getString("turn")).ordinal();
                        JSONArray items = json.getJSONArray("items");
                        for(int i = 0; i < items.length(); ++i) {
                            JSONObject item = items.getJSONObject(i);
                            int way1_idx = item.getInt("way1");
                            int idx_in_way1 = item.getInt("idx_in_way1");
                            ways[way1_idx].get(idx_in_way1).type = com.example.checkers.Othermode.Item.ITEM_TYPE.valueOf(item.getString("type"));
