...
public class MyRepository {
    ...
    private static class loadAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        private WayDao mAsyncWayDao;
        private ItemDao mAsyncItemDao;
        private ArrayList<com.example.checkers.Othermode.Item>[] mWays;
        private Game mGame;
        loadAsyncTask(String gameName, GameDao gDao, PositionDao pDao, WayDao wDao, ItemDao iDao) {
            ...
            mAsyncWayDao = wDao;
            mAsyncItemDao = iDao;
            mWays = new ArrayList[com.example.checkers.Othermode.Item.WAYS_COUNT];
            for (int i = 0; i < mWays.length; ++i) // Максимум 8 клеток/шашек на одной диагонали
                mWays[i] = new ArrayList<com.example.checkers.Othermode.Item>(8);
        }
        @Override
        protected Void doInBackground(final Void... params) {
            ...
            for (int i = 0; i < _ways.size(); ++i)
            {
                ...
                for (int j = 0; j < _items.size(); ++j)
                {
                    ...
                    com.example.checkers.Othermode.Item item = new com.example.checkers.Othermode.Item(_item.mViewId,
                    ...
                    LOOP:
                    {
                        for (int q = 0; q < mWays.length; ++q)
                        {
                            ...
                            {
                                com.example.checkers.Othermode.Item added = mWays[q].get(s);
                                if (item.id == added.id) {
                                    item = added;
                                    break LOOP;
                                }
