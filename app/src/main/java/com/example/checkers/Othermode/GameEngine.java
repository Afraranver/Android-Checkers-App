...
import com.example.checkers.Othermode.Item.ITEM_TYPE;
...
public class GameEngine {
    ...
    {
        MyApplication.getCurrentActivity().findViewById(selectedFirstItem.id).setBackgroundResource(R.drawable.black_square_br);
        if (isJump) {
            for (int i = 0; i < jumps.size(); ++i) {
                LinkedList<Item> items = jumps.get(i);
                if (items.get(0).id == selectedFirstItem.id) {
                    for (int j = 2; j < items.size(); ++j) {
                        MyApplication.getCurrentActivity().findViewById(items.get(j).id).setBackgroundResource(R.drawable.black_square_br);
                    }
                }
            }
        } else { //isMove
            ...
            if (items.get(0).id == selectedFirstItem.id) {
                ...
                for (int j = 2; j < items.size(); ++j) {
                    ...
                    if (toItem.id == id) // Можем бить на клетку с идентификатором id?
                    {
                        ...
                        from.setImageResource(R.drawable.black_square_br);
                        from.setTag(R.drawable.black_square_br);
                        ...
                        middle.setImageResource(R.drawable.black_square_br);
                        middle.setTag(R.drawable.black_square_br);
                        return true;
                    }
                }
            }
    ...
    public void BackMove(Activity activity, int backmove)
    {
        turn = prevMove.turn;
        if (turn == Item.ITEM_TYPE.white) {
            backWhite +=1;
        ...
        if(turn == Item.ITEM_TYPE.white && backWhite <= 3 || turn == ITEM_TYPE.black && backBlack <=3){
            ...
            Toast.makeText(activity, "You have exceeded your Undo!!", Toast.LENGTH_SHORT).show();
        }
        ...
        for(int i = 0; i < ways.length; ++i)
        {
            ...
            for(int j = 0; j < way.size(); ++j) {
                ...
                if (item.type == Item.ITEM_TYPE.black)
                {
                    ...
                    ib.setImageResource(R.drawable.black_square_br);
                    ib.setTag(R.drawable.black_square_br);
                }
            }
        }
    }
